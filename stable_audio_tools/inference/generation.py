...
def generate_diffusion_cond(
    ...
    LOG.debug(conditioning_inputs["input_concat_cond"].shape)
    ...
    if init_audio is not None:
        ...
        # For other cases, you might need to prepare it as before
        if 'degraded_audio' not in conditioning[0]:
             in_sr, init_audio = init_audio # This line is problematic if init_audio is not a tuple
             io_channels = model.io_channels
    ...
    if clean_audio is not None and metrics_every > 0:
        ...
        if not can_calculate_metrics:
            ...
            for i, steps_i in enumerate(step_milestones):
                ...
                # Run sampler
                if diff_objective == "v":
                    all_args = {**sampler_kwargs, **conditioning_inputs, "cfg_scale": cfg_scale, "dist_shift": model.dist_shift, "batch_cfg": True, "rescale_cfg": True}
                    ...
                    all_args = {**sampler_kwargs, **conditioning_inputs, **negative_conditioning_tensors, "cfg_scale": cfg_scale, "dist_shift": model.dist_shift, "batch_cfg": True, "rescale_cfg": True}
    ...
    if model.pretransform is not None and not return_latents:
        ...
        sampled = model.pretransform.decode(sampled)
    LOG.debug(f"Sampled shape: {sampled.shape}")
    ...
    if clean_audio is not None and not return_latents:
        ...
        if model.pretransform:
            ...
            degraded_audio_latent = model.pretransform.encode(degraded_audio_tensor)
            degraded_audio_tensor = model.pretransform.decode(degraded_audio_latent)
            LOG.debug(f"Shapes: clean_audio_tensor: {clean_audio_tensor.shape}, clean_audio_latent: {clean_audio_latent.shape}, degraded_audio_tensor: {degraded_audio_tensor.shape}, degraded_audio_latent: {degraded_audio_latent.shape}")
        else:
            clean_audio_latent = None
            degraded_audio_latent = None
